...
class ChildParentTest extends KWBaseTestCase
{
     ...
     * - table
     */
    const CHILD_PARENTS = 'api/v1/kw/child-parents/';
    const CHILD_PARENTS_TABLE = 'child_parents';

    /**
     * @test
     */
    public function api_v1_child_parentsにGETメソッドでアクセスできる()
    {
        $response = $this->get(self::CHILD_PARENTS);
     ...
     * @test
     */
    public function api_v1_child_parentsにGETメソッドでアクセスするとJSONが返却される()
    {
        $response = $this->get(self::CHILD_PARENTS);
     ...
     * @test
     */
    public function api_v1_child_parentsにGETメソッドで取得できるユーザー情報のJSON形式は要件通りである()
    {
        $response = $this->get(self::CHILD_PARENTS);
        $books = $response->json();
        $book = $books[0];
