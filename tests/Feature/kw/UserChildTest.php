...
class UserChildTest extends KWBaseTestCase
{
     ...
     * @test
     */
    public function api_v1_user_childrenにGETメソッドでアクセスできる()
    {
        $response = $this->get(self::USER_CHILDREN);
     ...
     * @test
     */
    public function api_v1_user_childrenにGETメソッド_indexでアクセスするとJSONが返却される()
    {
        $response = $this->get(self::USER_CHILDREN);
     ...
     * @test
     */
    public function api_v1_user_childrenにGETメソッド_indexで取得できるユーザー情報のJSON形式は要件通りである()
    {
        $response = $this->get(self::USER_CHILDREN);
        $userChildren = $response->json();
        $userChild = $userChildren[0];
        $this->assertSame([
            'id',
            'sex_id',
            'icon',
            ...
            'last_kana',
            'birth_day'
        ], array_keys($userChild));
    }

    /**
     * @test
     */
    public function api_v1_user_childrenにGETメソッドで取得できるユーザー情報は10件である()
    {
        $response = $this->get(self::USER_CHILDREN);
