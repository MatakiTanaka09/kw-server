...
final class EloquentEventDetailRepository implements EventDetailRepositoryInterface
{
    ...
    public function postEventDetail(EventDetailRequest $req, $eventDetail)
    {
        ...
        $eventDetail->handing          = $req->json('handing');
        $eventDetail->started_at       = $req->json('started_at');
        $eventDetail->expired_at       = $req->json('expired_at');
        $eventDetail->capacity_members = $req->json('capacity_members');
        $eventDetail->event_minutes    = $req->json('event_minutes');
        $eventDetail->target_min_age   = $req->json('target_min_age');
        $eventDetail->target_max_age   = $req->json('target_max_age');
        $eventDetail->parent_attendant = $req->json('parent_attendant');
        $eventDetail->price            = $req->json('price');
        $eventDetail->cancel_policy    = $req->json('cancel_policy');
        $eventDetail->cancel_deadline  = $req->json('cancel_deadline');
        $eventDetail->pub_state        = $req->json('pub_state');
        $eventDetail->arrived_at       = $req->json('arrived_at');
